 ...
 *   EAGLE_API_URL  - URL base do Eagle zAPI (ex: https://ujnnjygkwznwdkjcqevp.supabase.co/functions/v1/api-gateway)
...
async function sendMessage(sessionId, phone, message, mediaUrl = null) {
  ...
  try {
    ...
    if (pathname === "/session/start" && req.method === "POST") {
      ...
      res.writeHead(200);
      return res.end(JSON.stringify({ success: true, message: `Session starting via ${mode}` }));
    }
    // POST /message/send
      res.writeHead(200);
